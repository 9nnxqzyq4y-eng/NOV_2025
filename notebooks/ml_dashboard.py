...
if not cfg["SUPABASE_URL"] or not cfg["SUPABASE_KEY"]:
    st.error(
        "Supabase no configurado. Define SUPABASE_URL y SUPABASE_SERVICE_ROLE_KEY en env o .streamlit/secrets.toml"
    st.stop()

# ---------- Clients ----------
@st.cache_resource(show_spinnerFalse)
...
# ---------- Data ----------
@st.cache_data(ttl60, show_spinnerFalse)
def read_table(schema: str, table: str) - pd.DataFrame:
    ...
    for c in [c for c in df.columns if "time" in c or "date" in c or c  "created_at"]:
        try:
            df[c]  pd.to_datetime(df[c], errors"coerce", utcTrue)
        except Exception:
            pass
...
pred_df  read_table(
    "ml", "predictions"
)  # id, loan_id, score|prediction_score, label, model_version, created_at
fb_df  read_table(
    "ml", "feedback"
)  # id, prediction_id, loan_id, outcome_label, correct, created_at
...
with c4:
    ...
    if (
        ...
        and {"id", "score"}.issubset(pred_df.columns)
        and "prediction_id" in fb_df.columns
    ):
        j  fb_df.merge(
            pred_df[["id", "score"]], left_on"prediction_id", right_on"id", how"left"
        )
        if not j.empty and "score" in j.columns and "correct" in j.columns:
            p  pd.to_numeric(j["score"], errors"coerce")
            y  pd.to_numeric(j["correct"], errors"coerce").fillna(0.0)  # 1correct, 0incorrect
            brier  (p - y) ** 2
            if brier.notna().any():
...
if not pred_df.empty and "score" in pred_df.columns:
    fig  go.Figure(go.Histogram(xpred_df["score"], nbinsx30))
    fig.update_layout(
        title"Prediction score distribution",
        height360,
        margindict(l10, r10, t40, b10),
        paper_bgcolorABACO["bg"],
        plot_bgcolorABACO["bg"],
        fontdict(colorABACO["fg"]),
    st.plotly_chart(fig, use_container_widthTrue)

# Label distribution
if not pred_df.empty and "label" in pred_df.columns:
    counts  pred_df["label"].value_counts().reset_index(names["label", "count"])
    fig  px.bar(counts, x"label", y"count", title"Prediction label distribution", height360)

# Rolling accuracy by day
if not fb_df.empty and "created_at" in fb_df.columns and "correct" in fb_df.columns:
    ts  (
        fb_df.assign(created_atpd.to_datetime(fb_df["created_at"], errors"coerce", utcTrue))
        .dropna(subset["created_at"])
        .assign(
            correctlambda x: pd.to_numeric(x["correct"], errors"coerce").fillna(0).astype(int)
        .set_index("created_at")
        .resample("1D")["correct"]
        .mean()
        .to_frame("acc")
        .rolling(windowmax(1, min(window_days, 120)), min_periods1)
        .reset_index()
    if not ts.empty:
        fig  go.Figure(go.Scatter(xts["created_at"], yts["acc"], mode"lines"))
        fig.update_layout(
            titlef"Accuracy trend (rolling {window_days}d)",
            yaxis_tickformat".0%",
            height360,
            margindict(l10, r10, t40, b10),
            paper_bgcolorABACO["bg"],
...
if not pred_df.empty:
    ...
    cols  [
        c
        for c in ["id", "loan_id", "score", "label", "model_version", "created_at"]
        if c in pred_df.columns
    ]
    view  (
        pred_df.sort_values("created_at", ascendingFalse)[cols].head(200).copy()
        if cols
        else pred_df.head(200)
    if "score" in view.columns:
        view["score"]  pd.to_numeric(view["score"], errors"coerce").map(
            lambda v: f"{v:.3f}" if pd.notna(v) else ""
    st.dataframe(view, use_container_widthTrue)
else:
    st.info("No predictions")

if not fb_df.empty:
    st.subheader("Recent feedback")
        for c in ["prediction_id", "loan_id", "outcome_label", "correct", "comments", "created_at"]
        if c in fb_df.columns
        fb_df.sort_values("created_at", ascendingFalse)[cols].head(200).copy()
        else fb_df.head(200)
...
if not mx_df.empty and "metrics" in mx_df.columns and "created_at" in mx_df.columns:
    ...
    m  mx_df.copy()
    m["created_at"]  pd.to_datetime(m["created_at"], errors"coerce", utcTrue)
    m["metrics_parsed"]  m["metrics"].apply(lambda x: x if isinstance(x, dict) else {})
    m["accuracy"]  pd.to_numeric(
        m["metrics_parsed"].apply(lambda d: d.get("acc")), errors"coerce"
    m["brier"]  pd.to_numeric(m["metrics_parsed"].apply(lambda d: d.get("brier")), errors"coerce")

    c1, c2  st.columns(2)
    if m["accuracy"].notna().any():
        fig  go.Figure(go.Scatter(xm["created_at"], ym["accuracy"], mode"lines+markers"))
            title"Accuracy over time",
            height320,
        c1.plotly_chart(fig, use_container_widthTrue)
    if m["brier"].notna().any():
        fig  go.Figure(go.Scatter(xm["created_at"], ym["brier"], mode"lines+markers"))
            title"Brier score over time (lower is better)",
        c2.plotly_chart(fig, use_container_widthTrue)
