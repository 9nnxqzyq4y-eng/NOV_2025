...
def get_model_version_id(model_name: str, version_number: int) - str | None:
    ...
    url  f"{SUPABASE_URL}/rest/v1/ml.model_versions?model_nameeq.{model_name}&version_numbereq.{version_number}&selectid"
...
def post_rows(table: str, rows: List[Dict]) - int:
    ...
    resp  requests.post(url, headersHEADERS, jsonrows)
...
def main() - None:
    ...
    if os.path.exists(predictions_csv):
        dfp  pd.read_csv(predictions_csv, dtypestr)
        rows  []
    ...
    if os.path.exists(outcomes_csv):
        dfo  pd.read_csv(outcomes_csv, dtypestr)
        rows  []
